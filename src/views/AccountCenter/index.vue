  ...
  <el-container style="position: static;height: calc(100vh - 51px);">
    ...
    >
      <el-menu
        ...
        style="height: 100%;padding-top: 15px;background-color: rgb(245, 247, 250);border-right: none;"
        @select="handleSelect"
      >
        <div class="userCenterSidebarTitle">
          账号中心
        </div>
        <el-menu-item
          v-for="(item, listIndex) in sidebarMenuList"
          :key="listIndex"
          :index="item.key"
          style="margin: 5px 0px; padding-left: 29px"
        >
          <div style="position: relative">
            <i class="el-icon-document" />
            <span slot="title">{{ item.value }}</span>
            <div
              v-if="hasNotDealApply && item.key === 'companyInfo'"
              style="position: absolute;width: 8px;height: 8px;border-radius: 50%;background-color: #FF3300;right: 75px;top: 10px;"
            />
          </div>
        </el-menu-item>
      </el-menu>
      ...
      >
        ...
        <div class="userCenterPageContent">
                  ...
                  <div class="userCenterUserInfo flex ai-center jc-between">
                    <div>
                      <div class="userCenterUserInfoItem">
                        <div class="userCenterUserInfoItemTitle">
                          姓名:
                        </div>
                        <span style="width: 100px;display: inline-block;">{{ userModel.userName }}</span>
                        <el-link
                          :underline="false"
                          class="userCenterUserInfoItemEdit"
                          @click="editPersonalInfo"
                        >
                          修改
                        </el-link>
          ...
          <template v-if="menuDefaultActive == 'safeInfo'">
            <div class="user-container">
              <div
                class="userCenterBaseContent"
                style="height: 301px; padding: 0 50px 0 50px"
              >
                <div
                  class="safetySettingItem"
                  style="border-bottom: 1px solid #d8d8d8"
                >
                  <i
                    class="el-icon-circle-check"
                    style="font-size: 38px;color: #49df66;float: left;margin-top: 16px;"
                  />
                  <div class="safetySettingItemTextContainer">
                    <div class="safetySettingItemTextTop">
                      登录密码
                    </div>
                    <div class="safetySettingItemTextBottom">
                ...
                <div class="safetySettingItem">
                  ...
                  <div class="safetySettingItemTextContainer">
                    ...
                    <div class="safetySettingItemTextBottom">
                      您已绑定了手机 <span style="color: #006fff">{{ userPhoneNumber }}</span>，您的手机为安全手机，可以找回密码和登录！
                    </div>
                  </div>
                  <div style="float: right; margin-top: 45px">
                    <el-button
          ...
          <template v-if="menuDefaultActive == 'companyInfo'">
            <el-row class="table">
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    ...
                    <el-link
                      v-if="scope.row.applyStatus == 3 || scope.row.applyStatus == 0"
                      :underline="false"
                      style="margin-left: 18px"
                      type="primary"
                      @click="checkAdmin(scope.row)"
                    >
                      查看管理员
                    </el-link>
                    <el-link
              ...
              <div
                v-if="showBtn"
                style="height: 50px; line-height: 150px; text-align: center"
              >
                <el-button
                  size="small"
                  style="width: 150px"
                ...
                >
                  <template slot-scope="scope">
                    <el-link
                      type="primary"
                      :underline="false"
                      @click="handleLinkToSpaceDetail"
                    >
                      {{ scope.row.privation ? "无限制" : scope.row.usableSpace }}
                    </el-link>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                >
                  <template slot-scope="scope">
                    <el-link
                      ...
                      type="danger"
                      @click="handleExitCompany(scope.row)"
                    >
                      退出企业
                    </el-link>
                    <el-link
                      v-if="userModel.isAdmin"
                      :underline="false"
                      style="margin-left: 18px; position: relative"
                      type="primary"
                      @click="handleCheckCustom(scope.row)"
                    >
                      <div
                        v-if="hasNotDealApply"
                        style="position: absolute;width: 8px;height: 8px;border-radius: 50%;background-color: #FF3300;right: -10px;"
                      /> 查看人员
                    </el-link>
                  </template>
                </el-table-column>
    ...
    />
    <companyDialogCom
      :dialog-visible.sync="companyDialogVisible"
      @reload="loadData"
    />
  </el-container>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      projectCreatorAvatorUrl: process.env.VUE_APP_BASE_API + "/api/home/GetimgFile?fileUrl=",
      companyApplyData: [],
      encrypt
    };
  },
