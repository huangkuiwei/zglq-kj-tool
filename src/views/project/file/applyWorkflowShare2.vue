<template>
  <div class="approve-dialog">
    <el-dialog title="发起下载流程" :visible="dialogVisible" width="455px" v-dialogDrag :modal="true"
      ...
      <el-form label-width="95px" label-position="left">
        <template v-if="roles.length > 0">
          ...
          <div v-for="(roleItem, k) in roles">
            <el-form-item :label="roleItem.currentRole" required label-width="200px" size="normal">
              <el-select style="margin-bottom:10px; width:190px;" v-model="roleItem.current"
       ...
       <div>
         <el-form-item label="下载文件类型" label-width="200px" size="normal">
           <el-checkbox-group v-model="downloadType">
             <el-checkbox :label="2" :key="2">下载bim文件</el-checkbox>
             <el-checkbox :label="1" :key="1">下载源文件</el-checkbox>
           </el-checkbox-group>
         </el-form-item>
       </div>
      </el-form>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :disabled="validateSubmit || !downloadType.length" :loading="loading" @click="saveAppliedWorkflowAll"
...
export default {
  ...
  methods: {
    ...
    saveAppliedWorkflowAll() {
      let downloadType = ''
      let relatedFiles = ''

      this.rowData.forEach(row => {
        relatedFiles = row.iuid

        // 文件
        if (row.fileSuffix) {
          if (this.isIModelFile(row) && row.fileSuffix.toLowerCase() !== '.bim') {
            if (this.downloadType.length === 1) {
              if (this.downloadType.includes(1)) {
                downloadType = '.'
              } else if (this.downloadType.includes(2)) {
                downloadType = '.bim'
              }
            } else {
              downloadType = '.,.bim'
        ...
        this.saveAppliedWorkflow(downloadType, relatedFiles)
      })

    },

    // 保存工作流到项目工作流表中(流程发起表)
    async saveAppliedWorkflow(downloadType, relatedFiles) {
      ...
      if (this.roles.map(item => item.current.length).indexOf(0) >= 0) {
        ...
        console.log('this.rowData', this.rowData)
        data.append('relatedFiles', relatedFiles);
        data.append('mainiuid', relatedFiles);
        ...
        data.append('module', 'download');
        var res = await projectFileApi.post("/api/Workflow/MainworkflowAdd", data);
        if (res.code == 1) {
          for (var i = 0; i < this.roles.length; i++) {
            this.roles[i].current.forEach(async (item) => {
              await this.startWorkflow(res.twodata, workflow, item, relatedFiles);
            })
          }

          let formData = new FormData();
          formData.append('MainIUID', res.twodata)
          formData.append('fileIUID', relatedFiles)
          formData.append('downloadType', downloadType)
          projectFileApi.post("/api/ProjectFile/AddworkflowFiledown", formData);
