  ...
  <div class="app-container background table-box">
    <!-- <el-row>
      <el-col :span="22" class="btn-group">
        <shareMutiple v-if="multipleSelection.length >= 1 && $shareMultipleVisible(multipleSelection)" :rows="multipleSelection" />
      </el-col>
    </el-row> -->
    <el-row style="margin-top: 20px" v-if="breadcrumb.length > 0">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <a @click="queryAll">全部文件</a>
        </el-breadcrumb-item>
    ...
    <el-row class="table">
      <el-table :data="tableData" class="t-table" :header-cell-style="$thStyle" style="margin-top: 20px; font-size: 13px" stripe height="100%" @selection-change="handleSelectionChange" @sort-change="sortChange" :default-sort="{ prop: 'createTime', order: 'descending' }">
        ...
        <el-table-column fixed="right" :label="$t('base.formLabel.operation')" width="120">
          <template slot-scope="scope">
            <el-tooltip style="margin-right: 10px;" effect="light" content="取消收藏" placement="top">
              <el-link :underline="false" icon="el-icon-collect" @click="cancelCollectionFile(scope.row)" style="color:#FFE039"></el-link>
            </el-tooltip>
            <el-tooltip v-if="scope.row.fileSuffix.toLowerCase() != ''" class="tableOpeColMargin" effect="light" content="版本" placement="top">
            ...
            <el-tooltip v-else disabled class="tableOpeColMargin">
              <i class="el-icon-version shrink" style="opacity: 0.4" />
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    async cancelCollectionFile(row) {
      let formData = [];
      formData.push({
        IUID: row.iuid,
        FileType: row.fileType,
      });
      var res = await commonApi.cancelCollect(formData);
      if (res.code == 1) {
    ...
    async collectionFile() {
      ...
      for (let key in this.multipleSelection) {
        formData.push({
          IUID: this.multipleSelection[key].iuid,
          FileType: this.multipleSelection[key].fileType,
        });
      }
      var res = await projectFileApi.post(
        "/api/Drawing/DrawingCollection",
        formData
      );
      if (res.code == 1) {
        this.$message({
    ...
    formatter(row) {
      if (
        ...
        row.fileSuffix.toLowerCase() != ".dgn"
      ) {
