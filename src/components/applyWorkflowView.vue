<template>
  <div class="approve-dialog">
    <el-dialog title="发起流程" :visible="dialogVisible" width="455px" v-dialogDrag :modal="true"
...
export default {
  ...
  methods: {
    ...
    async getAllWorkflows() {
      ...
      queryForm.append('isremove', true)
    ...
    async saveAppliedWorkflow() {
      ...
      if (this.roles.map(item => item.current.length).indexOf(0) >= 0) {
        ...
        var res = await projectFileApi.post("/api/projectworkflow/projectworkflowAdd", data);
        if (res.code == 1) {
