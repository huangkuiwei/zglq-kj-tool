    ...
    <el-container>
      <el-main>
        <el-container class="Hight100">
          ...
          <el-main>
            <p v-html="this.DataText" style="padding-left:10px;height:12%;overflow-y:scroll;">{{this.DataText}}</p>
          </el-main>
        </el-container>
      </el-main>
    </el-container>
...
<script>
  export default {
    ...
    methods: {
      ...
      open: function () {
        this.socket.send('GetInfo') //获取日志信息数据
      },
      error: function () {
        this.loading = false;
      },
      getMessage: function (msg) {
        ...
        if (list.Top == "InfoDATE") {
          this.tableData = list.Data;
          this.loading = false;
          console.log(JSON.parse(msg.data))
        } else if (list.Top == "InfoDATEText") {
      ...
      tableRowClick(row) {
        this.socket.send("Info-" + row.Name)
      }
    },
    destroyed() {
      // 销毁监听
